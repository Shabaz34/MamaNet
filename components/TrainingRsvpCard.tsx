...
import { CalendarClock, Check, ChevronDown, ChevronUp, Loader2, Settings, UserPlus, X } from 'lucide-react';
      ...
      <div className="p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {isCaptain && (
            <button
              ...
              aria-label="הגדרות אימונים"
              aria-expanded={settingsOpen}
              className="w-9 h-9 rounded-xl flex items-center justify-center text-slate-400 hover:bg-violet-50 hover:text-violet-600 transition shrink-0"
            >
              <Settings size={17} />
            </button>
          )}
        ...
        {isCaptain && next && (
          <div className="border-t border-violet-50 pt-4 flex flex-col gap-2.5">
            <p className="text-xs font-bold text-slate-500">משלימות (קפטנית)</p>
...
function TrainingSettingsForm({
  ...
  next: ReturnType<typeof useNextTraining>;
  onDone: () => void;
}) {
  ...
  const [overrideDate, setOverrideDate] = useState(next?.dateKey ?? '');
  const [overrideTime, setOverrideTime] = useState(next?.time ?? '');
  ...
  async function handleSaveDefault(e: FormEvent) {
    ...
    try {
      await saveTrainingSettings(teamCode, { weekday, time }, captainUid);
    } catch (err) {
  ...
  async function handleSaveOverride(e: FormEvent) {
    ...
    try {
      await saveTrainingOverride(teamCode, next.defaultDateKey, overrideDate, overrideTime, captainUid);
      onDone();
    } catch (err) {
  ...
  return (
    <div className="border-t border-violet-100 bg-violet-50/40 p-5 flex flex-col gap-5 animate-[fade-in_0.2s_ease-out]">
      <form onSubmit={handleSaveDefault} className="flex flex-col gap-3">
        <p className="text-xs font-bold text-slate-600">ברירת מחדל שבועית</p>
        <div className="flex gap-2">
          <select
            value={weekday}
            onChange={(e) => setWeekday(Number(e.target.value))}
            className="flex-1 rounded-xl border border-slate-200 px-3 py-3 min-h-[48px] text-sm text-slate-800 bg-white focus:outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100 transition"
          >
      ...
      {next && (
        <form onSubmit={handleSaveOverride} className="flex flex-col gap-3 border-t border-violet-100 pt-4">
      ...
      <button
        type="button"
        onClick={onDone}
        className="flex items-center justify-center gap-1.5 text-xs font-bold text-slate-400 hover:text-slate-600 transition"
      >
        <ChevronDown size={14} />
        סגירה
      </button>
